...
<div class="container">
    ...
    <form class="main-form" name="add-type-form" enctype="multipart/form-data" id="add-type-form" method="post" action="{{route('store-category')}}">
        ...
        <div class="form-group">
            <input type="number" min="0" required class="form-control" id="title" name="type[percentage]" value="{{ (isset($type) ? $type["percentage"] : "") }}" placeholder="Category Rent Percentage(%)">
            @if (isset($errs['name']))
            <div class="error_msg">*{{ $errs['name'] }}</div>
            @endif
        </div>
        <div class="form-group">
            <input type="number" min="0" required class="form-control" id="title" name="type[commission_percentage]" value="{{ (isset($type) ? $type["commission_percentage"] : "") }}" placeholder="Category Commission Percentage(%)">
            @if (isset($errs['name']))
            <div class="error_msg">*{{ $errs['name'] }}</div>
            @endif
        </div>
