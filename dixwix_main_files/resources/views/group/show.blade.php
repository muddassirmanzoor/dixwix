...
<div class="container p-0">
    ...
    <div class="tab-container">
        <div class="tab-buttons tab d-flex flex-column flex-md-row">
            ...
            <button class="tab-button tablinks {{ $group['status'] ? '' : 'd-none' }} {{ $active_tab == 'requests' ?  'active' : ''}}" data-tab="tab2">Participants</button>
            <button style="width:23%" class="tab-button {{ $group['status'] ? '' : 'd-none' }} tablinks {{ $active_tab == 'return-requests' ?  'active' : ''}}" data-tab="tab4">Rental & Returns</button>
            <button style="width:23%" class="tab-button {{ $group['status'] ? '' : 'd-none' }} tablinks {{ $active_tab == 'community' ? 'active' : '' }}" data-tab="tab5">Community</button>
            <button style="width:23%" class="tab-button  tablinks " data-tab="tab6">Group Rental Rules</button>
                        ...
                        <div class="col-md-4 group-show-qr-code-section p-0 {{ !empty($group['to_be_deleted_at']) ? 'text-right' : 'd-flex' }} justify-content-end">
                            ...
                            <ul class="list-group list-group-horizontal-md group-main-actions">
                                ...
                                <li class="list-group-item">
                                    ...
                                    <button style="margin-top: 5px; border-radius: 5px !important;" id="shareBtn_{{ $group['id'] }}" class="dark-btn btn link_with_img" onclick="copyToClipboard('{{ route('show-group', ['id' => $group['id']]) }}', 'shareBtn_{{ $group['id'] }}')">
                                        <img src="{{ url('assets/media/sharing-icon-png-14.jpg') }}"> Share
                                    </button>
                                </li>
                                @endif
                            </ul>
...
{{--                                    <th scope="col" style="background:#094042;">Item Description</th>--}}
                                        ...
                                        <th scope="col" style="background:#094042;">Copies</th>
                                        <th scope="col" style="background:#094042;">Status</th>
                                        <th scope="col" style="background:#094042;">
                                            @if(collect($group['books'])->contains(fn($book) => $book['created_by'] !== auth()->user()->id))
                                            Due Date
                                            @endif
                                        </th>
                                        <th scope="col" style="background:#094042;">Requests</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><a style="text-decoration: none;" href="{{ route('show-item', ['id' => $book['id']]) }}">{{ $book['name'] }}</a></td>
{{--                                        <td>{{ $book['description'] }}</td>--}}
                                        <td>{{ $book['rent_price'] }}</td>
                                        <td>{{ isset($book['category']) && isset($book['category']['name']) ? $book['category']['name'] : 'No Category' }}</td>
                                        ...
                                        <td style="text-align: center">
                                            ...
                                            <span class="badge {{ $book['copies'] == 0 ? 'badge-danger' : $color_class }} px-4 py-2">
                                                {{ ($book['copies'] == 0) ? ($book['reservation_deleted'] ? 'Requests cancelled all booked' : "Rented") : $status }}
                                            </span>
                                            ...
                                            @endforeach
                                            @if($is_reserved)
                                            <span class="badge badge-warning px-4 py-1"><span class="badge badge-success px-2 py-1">{{ $pending_count }} </span> <a href="#tab4" style="text-decoration: none;color: #212529;">Approval pending</a></span>
                                            @elseif($book['copies'] == 0)
                                            <span class="badge badge-success px-4 py-1">Booked</span>
                                            @else
                                            <span class="badge badge-primary px-4 py-1">Available</span>
                                            @endif
                                            @if($pending_return>0)
                                            &nbsp;<br><span class="badge badge-warning px-4 py-1"><span class="badge badge-success px-2 py-1">{{ $pending_return }} </span> <a href="#tab4" style="text-decoration: none;color: #212529;">Return Approval pending</a></span>
                                            @endif
                                            @endif
                                            @endif
                                        </td>
                                        ...
                                        <td style="text-align: center">
                                            @if ($book['created_by'] !== auth()->user()->id && $book['due_date']!="")
                                                @php
                                                    $dueDate = \Carbon\Carbon::parse($book['due_date']);
                                                    $isOverdue = $dueDate->isPast(); // Check if the due date is past
                                                @endphp
                                        ...
                                        <td style="text-align: center">
                                            @if ($book['created_by'] !== auth()->id())
                                                ...
                                                <!-- @php
                                                    $dueDate = !empty($book['due_date']) ? \Carbon\Carbon::parse($book['due_date']) : null;
                                                    $isOverdue = $dueDate ? $dueDate->isPast() : false;
                                                @endphp

                                                @if($isOverdue)
                                                    <a href="javascript:void(0)" onclick="setBookStatus({{ $book['id'] }}, {{ $group['id'] }},'reserved', 1)" class="btn-sm mt-2 btn btn-green"> Renewal</a>
                                                @endif -->
                        ...
                        @else
                        <div class="alert alert-warning px-4 mt-2 py">
                            <h2>You don't have permission to see the items until you join the group.</h2>
                        </div>
                        @endif
                    </div>
                </div>
        ...
        <div class="tab-content {{ $group['status'] ? '' : 'd-none' }} {{ $active_tab == 'requests' ?  'active' : ''}}" id="tab2">
            <div class="container p-0">
                <div class="row pt-4">
        ...
        <div class="tab-content {{ $group['status'] ? '' : 'd-none' }} {{ $active_tab == 'return-requests' ?  'active' : ''}}" id="tab4">
            <div class="container p-0">
                <div class="heading mt-4 d-flex" style="justify-content: space-between">
                    ...
                    <button id="refreshBtn" style="display: none; margin-left:50%" type="button" class="btn btn-primary" onclick="refreshTable()">Back</button>
        ...
        <div class="tab-content {{ $group['status'] ? '' : 'd-none' }} {{ $active_tab == 'community' ? 'active' : '' }}" id="tab5">
         ...
         <div class="tab-content {{ $group['status'] ? '' : 'd-none' }} {{ $active_tab == 'rental-rules' ?  'active' : ''}}" id="tab6">
            <div class="container p-0">
                <div class="heading mt-4 d-flex" style="justify-content: space-between">
                    <h2>Group Rental Rules</h2>
              </div>
                <div class="item search-result">
                    <div class="container">
                        <div class="row group-desc-row">
                            <div class="col-12 members-section">
                                <div class="parg">
                                   <h5> <strong>1. Respect & Integrity: </strong>Treat all members with respect, honesty, and truthfulness in actions and interactions. </h5> 	
	                               <h5> <strong>2. Positive Contributions:</strong> Foster a supportive environment by contributing positively and refraining from negative comments or opinions.</h5>	
	                               <h5> <strong>3. Item Responsibility:</strong> Ensure items shared are in good, safe working condition and handle all borrowed items as if they were your own.	</h5>
	                               <h5> <strong>4. Conflict Resolution:</strong>  Report and Replace any damaged items.Resolve disputes amicably, seeking help from community members if needed.</h5>	
	                               <h5> <strong>5. Adherence to Guidelines:</strong> Follow all group rules and guidelines to maintain a safe and thriving community.</h5>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="modal-body" id="modal_body" style="overflow-y: scroll; max-height: 450px;">
                    ...
                    <div class="table-group-details">
                        <table id="items_table_group" class="table">
                            <thead>
                                <tr>
                                    <th scope="col" style="background:#094042;">Copy#</th>
                                    <th scope="col" style="background:#094042;">Copy Name</th>
                                    <th scope="col" style="background:#094042;">Reserved Status</th>
                                    <th scope="col" style="background:#094042;">Reserved By</th>
                                    <th scope="col" style="background:#094042;">Member Trust Score</th>
                                    <th scope="col" style="background:#094042;">Due Date</th>
                                    <th scope="col" style="background:#094042;" class="text-center">Action</th>
                                </tr>
                            </thead>
    ...
    function refreshTable() {
        ...
        // Set or replace the 'tab' query parameter
        url.searchParams.set('tab', 'return-requests');
