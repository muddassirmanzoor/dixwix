<div class="inner_content">
  ...
  <form method="POST" id="buld-edit-items-form" action="{{ route('bulk-update-items-all') }}">
        @csrf
    <input type="hidden" name="items" value="{{ json_encode($items) }}">
    <table class="table table-bordered">
            <theader > <h3 style="margin-left:30%; "> Select Values For All the Items at Once. </h3>
            </theader>  
            <thead>
                <tr>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <select name="all_item_group" class="form-control">
                    ...
                    <td>
                        <select name="all_item_category" class="form-control category-select" data-item-id=>
                            <option value="" data-percent="0">Select Category</option>
                            @foreach ($categories as $category)
                            <option value="{{ $category->id }}" data-percent="{{ $category->percentage ?? 0 }}" >
                                {{ $category['name'] }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <td>
                        <input type="number" name="all_item_price" value="" class="form-control price-input" data-item-id="">
                    </td>
                    <td>
                        <input type="number" name="all_rent_price" value="" class="form-control rent-price-input" data-item-id="">
                    </td>
                    <td>
                        <select name="all_status" class="form-control">
                            <option value="">Select Status</option>
                            <option value="disable" >Disabled</option>
                            <option value="maintenance" >Enabled</option>
                        </select>
                    </td>
                    <td>
                        <select name="all_item_condition" class="form-control">
   ...
   <br> 
    ...
    <form method="POST" id="buld-edit-items-form" action="{{ route('bulk-update-items') }}">
        ...
        <table class="table table-bordered">
          <theader > <h3 style="margin-left:30%; "> Select Values For Items Seperately. </h3>
            </theader>
            <thead>
                <tr>
                    <th>Name</th>
                ...
                <tr>
                    ...
                    <td>
                        <select name="items[{{ $index+1 }}][status_options]" class="form-control">
                            ...
                            <option value="maintenance" {{ $item->status_options == 'maintenance' ? 'selected' : '' }}>Enabled</option>
                        </select>
                    </td>
                    <td>
