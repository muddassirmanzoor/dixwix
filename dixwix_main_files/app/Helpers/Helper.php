...
function SendEmail($sendto,$user_name ,$title, $description)
{
 //   $formData = ["message" => $title, "email" => $desription];
 //   Mail::to($sendto)->send(new MailService($formData));
    try {
        $formData = ["user_name"=>$user_name, "message" => $title, "email" => $description];
       // Mail::to($sendto)->send(new MailService($formData));
      Mail::to($sendto)->send(new MailService($formData));
        return "Email sent!" ;
    } catch (\Exception $e) {
...
if (!function_exists('getCampaign')) {
    function getCampaign($id)
    {
        ...
        if (empty($campaign) || !isset($campaign['data'])) {
            // Return fallback structure instead of redirect
