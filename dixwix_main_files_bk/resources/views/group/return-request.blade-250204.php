    ...
    <table id="items_table_requests" class="table table-striped table-rounded">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td> <span class="badge badge-{{ $entry['original_condition'] == 'yes' ? 'success' : 'warning' }}">{{ $entry['original_condition'] }}</span></td>
                <td>{{ $book['item_id'] }}</td>
                <td>{{ $requestedBy }}</td>
                ...
                <td>{{ $entry['is_reserved'] == 2 ? 'Rental' : ($entry['state'] == 'return-request' ? 'Return' : '') }}</td>
                <td>
                    @if($entry['is_reserved'] == '2' && $entry['reserved_by'] != auth()->id())
                    <div class="btn-group">
